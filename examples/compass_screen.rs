...
use sensehat::{SenseHat, PixelColor, Screen};
use sensehat_screen::{PixelFrame, Rotate};
...
fn main() {
    ...
    let up = right.rotate(Rotate::Ccw90);
    let up_left = right_up.rotate(Rotate::Ccw90);
    let up_right = right_down.rotate(Rotate::Ccw90);

    let left = right.rotate(Rotate::Ccw180);
    let left_up = right_down.rotate(Rotate::Ccw180);
    let left_down = right_up.rotate(Rotate::Ccw180);

    let down = up.rotate(Rotate::Ccw180);
    let down_left = up_right.rotate(Rotate::Ccw180);
    let down_right = up_left.rotate(Rotate::Ccw180);
