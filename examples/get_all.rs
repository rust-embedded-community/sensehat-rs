...
fn main() {
    ...
    let temp = sense_hat.get_temperature_from_humidity()
        .expect("Couldn't get temp");
    println!("It's {} on the humidity sensor", temp);
    let temp = sense_hat.get_temperature_from_pressure()
        .expect("Couldn't get temp");
    ...
    loop {
        let orientation = sense_hat.get_orientation()
            .expect("Couldn't get orientation");
        println!("Fusion orientation: {}, {}, {}",
                 orientation.roll,
                 orientation.pitch,
                 orientation.yaw);
        ...
        if let Ok(orientation) = sense_hat.get_gyro() {
            println!("Gyro orientation :  {}, {}, {}",
                     orientation.roll,
                     orientation.pitch,
                     orientation.yaw);
        }
        if let Ok(orientation) = sense_hat.get_accel() {
            println!("Accel orientation:  {}, {}, {}",
                     orientation.roll,
                     orientation.pitch,
                     orientation.yaw);
        }
